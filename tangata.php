
<?php require_once('inc/head.html'); ?>

<body id="tangata">

	<?php require_once('inc/header.html'); 
		require_once('inc/bd-connect.php');
	?>

	<main>
		<div class="container">
			<div class="row position-relative background-image mb-5">
				<div class="overlay"></div>
				<div class="col-12 gradient pt-5">
					<h1>Donnez une nouvelle dimension à l'inclusion !</h1>
				</div>
			</div>
			<div class="row description mb-5">
				<div class="col-md-4 col-sm-6 col-xs-12">
					<i class="fas fa-compress-arrows-alt blue-circle-border"></i>
					<p>Votre marque employeur favorisé par l'inclusion</p>
				</div>
				<div class="col-md-4 col-sm-6 col-xs-12">
					<i class="fas fa-users blue-circle-border"></i>
					<p>Un réseau d'experts handicap pour booster votre attractivité</p>
				</div>
				<div class="col-md-4 col-sm-6 col-xs-12">
					<i class="fas fa-handshake blue-circle-border"></i>
					<p>Des profils qualifiés pour votre recrutement</p>
				</div>
			</div>
			<div class="row position-relative tangata-help">
				<div class="overlay"></div>
				<div class="col-12 no-padding">
					<h2>Tangata vous aide à valoriser votre politique inclusive pour l'emploi des travailleurs handicapés</h2>
				</div>
			</div>
			<div class="row see-entreprises pb-5 py-5">
				<div class="col-12 no-padding">
					<h2 class="mb-5">Découvrez les entreprises Tangat'Addicts</h2>
					<a href="entreprises" class="btn btns btn-blue">Voir les entreprises</a>
				</div>
			</div>
		</div>
	</main>

	<?php require_once('inc/footer.html'); ?>